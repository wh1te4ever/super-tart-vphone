...
apticket_der_path = f"/mnt5/{bootManifestHash}/System/Library/Caches/apticket.der"
os.system(f"tools/sshpass -p 'alpine' scp -q -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -P 2222 root@127.0.0.1:{apticket_der_path} .")

# ========= Grab & patch kernelcache =========
...
# sign
os.system("pyimg4 img4 create -p krnl.im4p -o kernelcache -m apticket.der")

# ========= apply patched kernelcache =========
# send to apply
kernelcache_path = f"/mnt5/{bootManifestHash}/System/Library/Caches/com.apple.kernelcaches/kernelcache"
kernelcache_path_bak = f"/mnt5/{bootManifestHash}/System/Library/Caches/com.apple.kernelcaches/kernelcache.bak"
if not check_remote_file_exists(kernelcache_path_bak): 
     print(f"Created backup ${kernelcache_path_bak}")
     remote_cmd(f"/bin/cp {kernelcache_path} {kernelcache_path_bak}")
# fix owner, permissions
os.system(f"tools/sshpass -p 'alpine' scp -q -r -ostricthostkeychecking=false -ouserknownhostsfile=/dev/null -o StrictHostKeyChecking=no -P 2222 kernelcache 'root@127.0.0.1:{kernelcache_path}'")
remote_cmd(f"/bin/chmod 0644 {kernelcache_path}")
remote_cmd(f"/usr/sbin/chown 0:0 {kernelcache_path}")
...
# sign
os.system("pyimg4 img4 create -p txm.im4p -o Ap,TrustedExecutionMonitor.img4 -m apticket.der")

# ========= apply patched kernelcache =========
# send to apply
txm_path = f"/mnt5/{bootManifestHash}/usr/standalone/firmware/FUD/Ap,TrustedExecutionMonitor.img4"
txm_path_bak = f"/mnt5/{bootManifestHash}/usr/standalone/firmware/FUD/Ap,TrustedExecutionMonitor.img4.bak"
if not check_remote_file_exists(txm_path_bak): 
     print(f"Created backup ${txm_path_bak}")
     remote_cmd(f"/bin/cp {txm_path} {txm_path_bak}")
# fix owner, permissions
os.system(f"tools/sshpass -p 'alpine' scp -q -r -ostricthostkeychecking=false -ouserknownhostsfile=/dev/null -o StrictHostKeyChecking=no -P 2222 Ap,TrustedExecutionMonitor.img4 'root@127.0.0.1:{txm_path}'")
remote_cmd(f"/bin/chmod 0644 {txm_path}")
remote_cmd(f"/usr/sbin/chown 0:0 {txm_path}")
...
# clean
os.system("rm apticket.der")
os.system("rm krnl.im4p")
os.system("rm kernelcache")
os.system("rm kcache.raw")
os.system("rm Ap,TrustedExecutionMonitor.img4")
